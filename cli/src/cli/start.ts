...
export function generateProxyBootstrapCode(): string {
  return [
    ...
    '    function wrapFetch(fetchFn) {',
    "      if (typeof fetchFn !== 'function' || fetchFn.__acpWrapped) return fetchFn;",
    '      var wrapped = function(input, init) {',
    '        if (!init || !init.dispatcher) {',
    '          init = Object.assign({}, init || {}, { dispatcher: proxyDispatcher });',
    '        }',
    ...
    '      return wrapped;',
    '    }',
    "    if (typeof undici.fetch === 'function') undici.fetch = wrapFetch(undici.fetch);",
    "    if (typeof globalThis.fetch === 'function') globalThis.fetch = wrapFetch(globalThis.fetch);",
    '  } catch (_) {}',
    '} catch (_err) {',
    '  /* best effort */',
    '}',
